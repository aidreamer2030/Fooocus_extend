...
def codeformer_gui():
    with.ge.Row():
        ...
        codeformer_background = gr.Checkbox(value=True, label="Background_Enhance"),
        codeformer_face_upsample = gr.Checkbox(value=True, label="Face_Upsample"),
        codeformfer_fidelity = gr.Slider(0, 1, value=0.5, step=0.01, label='Codeformer_Fidelity (0 for better quality, 1 for better identity)')
    return codeformer_enabled,codeformer_background,codeformer_face_upsample,codeformfer_fidelity
def check_ckpts():
    pretrain_model_url = {
...
def face_restoration(img, background_enhance, face_upsample, upscale, codeformer_fidelity, upsampler, codeformer_net, device):
    ...
    try: # global try
        ...
        return None, None
def codeformer_process(source_img,codeformer_background,codeformer_face_upsample,codeformfer_fidelity):
    check_ckpts()
    ...
    codeformer_net.eval()     
    result_image = cv2.cvtColor(np.array(source_img), cv2.COLOR_RGB2BGR)
    result_image = face_restoration(result_image, 
                                      codeformer_background, 
                                      codeformer_face_upsample, 
                                      1, 
                                      codeformfer_fidelity,
                                      upsampler,
                                      codeformer_net,
                                      device)
